...
- The canonical team repository is **`abdulhamid-n/salen-traffic-events`** on GitHub. Jamoliddin is a collaborator. The authenticated local checkout is `D:\wiut hackathon\code\salen-traffic-events-main`.
...
- EDA for all four sample clips is present in the canonical checkout.
...
D:\wiut hackathon\code\salen-traffic-events-main # canonical team repo
```
...
git clone https://github.com/abdulhamid-n/salen-traffic-events.git
cd salen-traffic-events
git pull origin main
```

The current canonical checkout is at `D:\wiut hackathon\code\salen-traffic-events-main`. Copy/cherry-pick only useful work from the staging repo after inspecting conflicts; do not blindly merge unrelated histories.

Before starting work in the canonical team repo:
1. Verify `git remote -v` and make sure `origin` points to `abdulhamid-n/salen-traffic-events`.
